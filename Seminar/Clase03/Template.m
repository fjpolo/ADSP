...
freqz(speech);
figure(2)
set(gcf, 'name', 'Frequency responce.Upsampled 32[kHz] wave')
title('Speech 32kHz')
freqz(speechUP)
...
%We unite the polyphase components to obtain the final filtered-upsampled
%signal
L=max(size(speech)); 
...
%La senal filtrada IIR parece sonar mejor
%The signal filtered with the IIR filter sounds better than the FIR
%filtered signal, with less distortion.
%If we compare figures 13 and 14, we can see that the signal fiter with the
%FIR filter we can see a bit of aliasing, what creates the distortion