...
x1a = xq1a*q;
figure(2)
set(gcf, 'name', '6-bit Mid-Rise quantization')
subplot(5,1,1),
plot(t,x1a)
...
x1b = xq1b*q;
figure(3)
set(gcf, 'name', '6-bit Mid-Tread quantization')
subplot(5,1,1),
plot(t,x1b)
title('Dequantized full range triangle wave')
%quantization error
...
figure(4)
set(gcf, 'name', '8-bit uLaw quantization')
subplot(5,1,1),
plot(t,x1_mu)
...
for i=1:length(sh)/3
   ...
   shu(i)= ((log(1+(255*abs(sh(i)/max(sh)))))/(log(1+255)));
   %shu(i) = (log(1+(255*(sh(i)/max(sh)))))/(log(1+255));
...
%*******************************************************************************
%The mid-rise can be seen as more accurate, because very small values are always
%quantized to half the quantization interval, so it reacts to very small input 
%values, but the mid-tread can save bit-rate because it always quantizes very 
%small values to zero. The expectation of the quantization error power for large
%signals stays the same for both types.
%uLaw has anyway a better performance for full range and under range signals,
%being a really strong advantage. The problem is, since it's not linear, some 
...
disp("Full range triangular wave mid-rise")
disp("SNR = "), disp(SNRx1a_db)

%20dB under full range triangular wave mid-rise
SNRx2a_db = 10*log((var(x2))/(var(qex2a)));
disp("20dB under full range triangular wave mid-rise")
disp("SNR = "), disp(SNRx2a_db)

%Full range sine wave mid-rise
SNRy1a_db = 10*log((var(y1))/(var(qey1a)));
disp("Full range sine wave mid-rise")
disp("SNR = "), disp(SNRy1a_db)

%20dB under full range sine wave mid-rise
SNRy2a_db = 10*log((var(y2))/(var(qey2a)));
disp("20dB under full range sine wave mid-rise")
disp("SNR = "), disp(SNRy2a_db)

%Audio mid-rise
SNRsa_db = 10*log((var(sa))/(var(qesa)));
disp("Audio mid-rise")
disp("SNR = "), disp(SNRsa_db)
...
disp("Full range triangular wave mid-tread")
disp("SNR = "), disp(SNRx1b_db)

%20dB under full range triangular wave mid-tread
SNRx2b_db = 10*log((var(x2))/(var(qex2b)));
disp("20dB under full range triangular wave mid-tread")
disp("SNR = "), disp(SNRx2b_db)

%Full range sine wave mid-tread
SNRy1b_db = 10*log((var(y1))/(var(qey1b)));
disp("Full range sine wave mid-tread")
disp("SNR = "), disp(SNRy1b_db)

%20dB under full range sine wave mid-tread
SNRy2b_db = 10*log((var(y2))/(var(qey2b)));
disp("20dB under full range sine wave mid-tread")
disp("SNR = "), disp(SNRy2b_db)

%Audio mid-tread
SNRsb_db = 10*log((var(sb))/(var(qesb)));
disp("Audio mid-tread")
disp("SNR = "), disp(SNRsb_db)
...
disp("Full range triangular wave")
disp("SNR = "), disp(SNRx1u_db)

%20dB under full range triangular wave
qex2u = x2_mu-x2;
SNRx2u_db = 10*log((var(x2))/(var(qex2u)));
disp("20dB under full range triangular wave")
disp("SNR = "), disp(SNRx2u_db)

%Full range sine wave
qey1u = y1_mu-y1;
SNRy1u_db = 10*log((var(y1))/(var(qey1u)));
disp("Full range sine wave")
disp("SNR = "), disp(SNRy1u_db)

%20dB under full range sine wave
qey2u = y2_mu-y2;
SNRy2u_db = 10*log((var(y2))/(var(qey2u)));
disp("20dB under full range sine wave")
disp("SNR = "), disp(SNRy2u_db)
...
qesu = sh_mu-sh2;
SNRsu_db = 10*log((var(sh))/(var(qesu)));
disp("Audio wave Mu-Law")
disp("SNR = "), disp(SNRsu_db)

%*******************************************************************************
%What stands out when comparing the SNRs of the sinusoidal and triangular waves?
%*******************************************************************************
%Although SNR is good for full range signals for linear compression, near 85dB,
%Mu-Law SNR is even better, reaching 134dB for full range signals and 101dB for
%not full range signals.